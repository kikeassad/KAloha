...
def animar(t_f, pkts, name, gif, n_nodos=11, tam_s=0):
    y_names = []
    ...
    """ """
    count = 0
    for i in np.linspace(5 + tam_s, t_f + tam_s, num=60):
        plt.clf()
        ax = plt.gca()
        ax.set_xlim(i - 5, i)
        ax.set_ylim(0, n_nodos + 1)
        for x in pkts:
            p = x.draw()
            ax.add_patch(p)
        plt.plot([], [])
        plt.yticks(np.arange(n_nodos + 2), y_names)
        plt.grid(True)
        plt.draw()
