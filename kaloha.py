...
def crear_dataset_l(n_datos, channel=False, bps=1e6, t_pkt="exp", delay=0, n_nodos=11, tam_s=8, p=1):
    S = Sim()
    C = Canal()
...
def crear_dataset_nn(n_datos, channel=False, bps=1e6, t_pkt="exp", delay=0, n_nodos=11, tam_s=8, p=1):
    S = Sim()
    C = Canal()
    Lambda = np.arange(0.01, 10.01, .01)
    slot = 1.0
    ...
    for l in Lambda:
        ...
        while m < t_datos:
            ...
            if tipo == "IntraSlot":
                result.append(state.get_state() + nodos[n].get_state()[:-1] + [nodos[n].n_pkts])
...
def animacion(l, t_f, p=1, bps=1e6, t_pkt="const", delay=0, n_nodos=51, tam_s=0, model=None, channel=True):
    S = Sim()
    C = Canal()
    slot = 1.0
        ...
        if tipo == "FinSlot":
            if nodos[n].fin_slot(t, S):
                ...
                if model is not None:
                    ...
                    val = model.module_(estado).to(device="cpu").detach().numpy()
                    p = prob(val)
                    nodos[n].set_prob(p)
...
def throughput(n_sim, Lambda, p=1, bps=1e6, t_pkt="const", delay=0, n_nodos=11):
    ...
    for l in Lambda:
        ...
        for con in range(n_sim):
            ...
            for x in nodos:
                S.push(Event(t, "FinSlot", x.nodo), t)
...
def throughput2(n_sim, Lambda, p=1, bps=1e6, t_pkt="const", delay=0, n_nodos=11):
    S = Sim()
    C = Canal()
    slot = 1.0
...
val = animacion(1, t, tam_s=8, n_nodos=10)

animar(t, val, "prueba", False, n_nodos=10, tam_s=8)

S = Sim()

for x in range(10):
    S.push(Event(1, "StartTx", x), 1)

while S.size() > 0:
    e = S.pop()
    print(e.tiempo, e.nodo)
"""