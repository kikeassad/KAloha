            ...
            if len(topologias) > 0:
                for top in topologias:
                    ...
                    for i in col:
                        """
                        if counter > 40:
                            print(i_units, h_units, h_layers)
                            reg = crear_modelo(i_units, h_units, h_layers)
                            l_data = 0
                            data = ambiente.crear_dataset_nn(50, channel=i, tam_s=word)
                            Y = data[:, -1]
                            Y = Y.reshape(-1, 1)
                        ...
                        l_data = 0
                        while l_data < 10 * p:
                            data = ambiente.crear_dataset_nn(10000, channel=i, tam_s=word)
                            Y = data[:, -1]
                            Y = Y.reshape(-1, 1)
                            Y = Y.astype(np.float32)
                            X = data[:, :-1]
                        ...
                        """Validación"""
                        data = ambiente.crear_dataset_nn(1000, channel=i, tam_s=word)
...
def graficar_error(ambiente, src="modelos/"):
    result = []
    col = [True, False]
    counter = 0
    for p in params:
                ...
                for top in topologias:
                    ...
                    for i in col:
                        ...
                        while l_data < 10 * p:
                            ...
                            del data
                            l_data += 10000
                            E_in.append(mean_squared_error(reg.predict(X), Y))
                            ...
                            E_out.append(mean_squared_error(reg.predict(X), Y))
...
for d in dir:
    ...
    s, ci_inf, ci_sup, e, e_inf, e_sup = throughput_kaloha(reg, channel, 100, i_units, Lambda)
